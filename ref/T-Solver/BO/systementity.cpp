...
QString writeZ3FailureLog(const QString &logic, const QString &errorMessage)
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qWarning() << "Failed to write Z3 log:" << file.errorString();
        return QString();
    }
...
{
    const char *raw = Z3_get_error_msg(ctx, err);
    QString message = QString::fromUtf8(raw ? raw : "unknown z3 error");
...
static QString formatRangeValue(double number)
{
...
static QString formatModelExpr(const z3::expr &value)
{
    if (value.is_bool()) {
                                                                                  ...
                                                                                  int timeoutMs)
{
    ...
    qDebug().noquote() << "[Z3][Incremental] collected declarations:" << session.declaredFunctions.size();
    qDebug().noquote() << "[Z3][Incremental] base logic:\n" << session.baseLogic;
                                           ...
                                           QMap<QString, QString> *modelOut) const
{
    ...
    if (!extraAssertions.isEmpty()) {
        ...
        qDebug().noquote() << "[Z3][Incremental] extra assertions:\n" << incrementalLogic;
        try {
            ...
            qDebug().noquote() << "[Z3][Incremental] declaration vector size:" << declVector.size();
            ...
            qWarning().noquote() << "[Z3][Incremental] parse failure:" << errorMessage;
            ensurePop();
            if (modelOut) {
                modelOut->clear();
            }
    ...
    if (!incrementalLogic.isEmpty()) {
        ...
        qDebug().noquote() << "[Z3][Incremental] combined logic length:" << combinedLogic.size();
    } else {
        g_currentZ3Logic = &session.baseLogic;
    }
    ...
    case z3::check_result::unknown:
        ...
        break;
    default:
        errorMessage = QString("Z3 返回未知结果");
        sat = false;
        break;
    }
...
void SystemEntity::solveConflictSets(const QString& modelDescription, const QList<TestItem>& testItemList)
{
    ...
    const QString rangeCode = buildVariableRangeCode(QStringList(), testItemList);
    if (!rangeCode.isEmpty()) {
        testCode += rangeCode;
    }
...
QList<resultEntity> SystemEntity::completeSolve(const QString& modelDescription, const QList<TestItem>& testItemList, int truncateMode, bool includeObs)
{
    ...
    //qDebug()<<"完整离线求解的输入=====================\n"<<unchangingCode + allComponentCode + testCode;
    if(z3Solve(unchangingCode + allComponentCode + testCode) == true) {
        qDebug()<<"无故障，求解结束";
...
QString SystemEntity::buildVariableRangeCode(const QStringList &variables,
                                             const QList<TestItem> &testItemList) const
{
...
QList<obsEntity> SystemEntity::creatObsEntity(const QList<TestItem>& testItemList) {
    ...
    for (const auto& testItem : testItemList) {
        ...
        const QString variable = testItem.variable.trimmed();
        const QString value = testItem.value.trimmed();
        if (variable.isEmpty() || value.isEmpty()) continue;
        double errorValue = 0.0;
        obsEntity obs;
        obs.setName("obs" + QString::number(testIndex));
        obs.setVariable(variable);
        obs.setConfidence(testItem.confidence);

        QString description;
        QString varType = obsVarsMap.value(variable);

        if(testItem.testType=="依赖功能"){
            if (!functionInfoMap.contains(variable)) {
                continue;
            }
            TestItem actC = functionInfoMap.value(variable).actuatorConstraint;
            const QString actuatorVar = actC.variable.trimmed();
            const QString actuatorValue = actC.value.trimmed();
            if (actuatorVar.isEmpty() || actuatorValue.isEmpty()) {
                continue;
            }
            if(value=="功能正常")description = QString("(assert (= %1 %2))").arg(actuatorVar, actuatorValue);
            else description = QString("(assert (not (= %1 %2)))").arg(actuatorVar, actuatorValue);
            ...
            //液压系统
            if(variable.endsWith("u"))errorValue=24*0.01;
            if(variable.endsWith("i"))errorValue=0.1;
            if (varType == "(Array Int Real)") {
                QString trimmedValue = value;
                ...
                if (trimmedValue.startsWith('(') && trimmedValue.endsWith(')')) {
                        ...
                        if (!positions[i].trimmed().isEmpty()) {
                            ...
                            if (ok) {
                                QString part = QString("(>= (select %1 %2) %3) "
                                        ...
                                        .arg(variable)
                                        .arg(i)
                                        .arg(formatRangeValue(num * lowerLimitRatio - errorValue))
                                        .arg(formatRangeValue(num * upperLimitRatio + errorValue));
                                components << part;
                            } else {
                    ...
                    for (int i = 0; i < 3; ++i) {
                                QString part = QString("(>= (select %1 %2) (* (select %3 %2) %4)) "
                                        ...
                                        .arg(i)
                                        .arg(trimmedValue)
                                        .arg(formatRangeValue(lowerLimitRatio))
                                        .arg(formatRangeValue(upperLimitRatio));
                        components << part;
                    }
                }
                    ...
                    if(match.hasMatch()) {
                        if(match.captured(1).length() > 0 && match.captured(4).length() > 0) {
                            ...
                            description = QString("(assert (and (%1 %2 %3)(%4 %2 %5)))")
                                    .arg(startCompare)
                                    .arg(variable)
                                    .arg(formatRangeValue(startValue))
                                    .arg(endCompare)
                                    .arg(formatRangeValue(endValue));
                            ...
                            description = QString("(assert (%1 %2 %3))")
                                    .arg(compare)
                                    .arg(variable)
                                    .arg(formatRangeValue(compareValue));
...
QString SystemEntity::creatSystemLinkCode(const QString& systemLinkDescription)
{
    ...
    for(QString description: ListRow){
        if(description.startsWith("rawsmt")){
            ...
            if(parameters.size()!=2)
                continue;
            QString code = "(assert (= (+ %A%.f %B%.f) 0))(assert (= %A%.p %B%.p))";

            code.replace("%A%", parameters[0]);
            code.replace("%B%", parameters[1]);
            systemLinkCode.append(code);
            qDebug()<<"connect2h:"<<code;
        }
            ...
            if(parameters.size()!=3)
                continue;
            QString code = "(assert (= (+ %A%.f %B%.f %C%.f) 0))(assert (= %A%.p %B%.p %C%.p))";
            code.replace("%A%", parameters[0]);
            code.replace("%B%", parameters[1]);
            code.replace("%C%", parameters[2]);
            systemLinkCode.append(code);
