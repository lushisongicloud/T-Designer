...
SystemStructure::SystemStructure(const QString& systemDescription, const QString& linkTextforCrop)
{
    ...
    QStringList cleanedComponents;
    if (!inputLinkTextforCrop.trimmed().isEmpty()) {
        QStringList rawComponents = inputLinkTextforCrop.split(',', QString::SkipEmptyParts);
        for (QString item : rawComponents) {
            item = item.trimmed();
            if (!item.isEmpty()) {
                cleanedComponents.append(item);
            }
        }
    }
    linkComponentSet = QSet<QString>::fromList(cleanedComponents);
    ...
    if(linkTextforCrop == "")
    {
        ...
        for (const QString &connectionLine : connectionLineList) {
            QStringList devices = getDeviceNamesInConnection(connectionLine);
            QStringList componentNames;
            for (const QString &device : devices) {
                QString baseName = device.split(".").first().trimmed();
                if (baseName.isEmpty()) {
                    continue;
                }
                componentNames.append(baseName);
                componentSetInConnection.insert(baseName);
            }
            if (!componentNames.isEmpty()) {
                portListInConnectionList.append(componentNames);
            }
        }
        // 遍历器件定义的每一行，提取器件名称
...
QStringList SystemStructure::getDeviceNamesInConnection(const QString &line) {
    ...
    while ((pos = rx.indexIn(line, pos)) != -1) {
        const QStringList rawDevices = rx.cap(1).split(",", QString::KeepEmptyParts);
        for (const QString &raw : rawDevices) {
