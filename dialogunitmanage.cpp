...
void DialogUnitManage::on_tableWidgetUnit_clicked(const QModelIndex &index)
{
    ...
    while(QueryVar2.next())
    {
        ...
        //==========加载端口==========
        //增加端口表 先从TermInfo表中按EquipmentTemplate_ID直接查询，然后再检查结果是否覆盖每个EquipmentTemplate的ConnNum，如果不能覆盖，则自动根据ConnNum进行解析并自动添加。
...
void DialogUnitManage::on_BtnBatchAutoGenerate_clicked()
{
    if (!m_batchDialog) {
        ...
        connect(m_batchDialog, &BatchAutoGenerateDialog::startRequested,
                this, [this](const QString &path, bool resume) { startBatchAutoGenerationNew(path, resume); });
        connect(m_batchDialog, &BatchAutoGenerateDialog::stopRequested,
                this, &DialogUnitManage::stopBatchAutoGenerationNew);
    }
...
void DialogUnitManage::startBatchAutoGenerationNew(const QString &logPath, bool resume)
{
    if (!m_batchDialog)
        return;
    ...
    // 检查 API Key
    if (!ensureDeepSeekApiKey()) {
        m_batchDialog->appendLog("DEEPSEEK_API_KEY 未设置，无法执行批量自动编写。");
        m_batchDialog->setRunning(false);
        return;
    }
    
    if (m_batchRunning) {
        m_batchDialog->appendLog("批量任务已在执行中。");
        return;
    }
    ...
    // 清理旧的 Worker Tabs
    m_batchDialog->closeAllWorkerTabs();
    m_batchDialog->appendLog(QString("===== 开始批量自动编写 ====="));
    m_batchDialog->appendLog(QString("日志文件: %1").arg(effectivePath));
    m_batchDialog->appendLog(QString("恢复模式: %1").arg(resume ? "是" : "否"));
    
    // 创建 Manager（如果不存在）
    if (!m_batchManager) {
        m_batchManager = new BatchAutoGenerateManager(T_LibDatabase, this);
        ...
        connect(m_batchManager, &BatchAutoGenerateManager::workerLogMessage,
                this, [this](int workerId, const QString &message) {
            // 只将 Worker 日志追加到对应的 Tab，不再显示在主日志中
            m_batchDialog->appendWorkerLog(workerId, message);
        });
        
        connect(m_batchManager, &BatchAutoGenerateManager::workerStreamDelta,
                this, [this](int workerId, const QString &delta) {
            // AI 流式输出追加到 Worker Tab 的流式输出区域
            m_batchDialog->appendWorkerStream(workerId, delta);
        });
        
        connect(m_batchManager, &BatchAutoGenerateManager::workerFinished,
                this, [this](int workerId, const EquipmentProcessResult &result) {
            QString statusStr = result.statusString();
            QString msg = QString("[BatchManager] Worker%1 完成: %2 - %3 (耗时: %4秒)")
                .arg(workerId)
                .arg(result.code)
                .arg(statusStr)
                .arg(result.elapsedSeconds);
            if (result.status == EquipmentProcessResult::Failed) {
                msg += QString(" - 错误: %1").arg(result.errorMessage);
            }
            m_batchDialog->appendLog(msg);
            
            // 在 Worker Tab 中添加完成标记
            QString workerMsg = QString("\n========== 处理完成 (耗时: %1秒) ==========\n状态: %2")
                .arg(result.elapsedSeconds)
                .arg(statusStr);
            if (result.status == EquipmentProcessResult::Failed) {
                workerMsg += QString("\n错误: %1").arg(result.errorMessage);
            }
            m_batchDialog->appendWorkerLog(workerId, workerMsg);
            
            // 注意：不关闭 Worker Tab，让用户可以查看完整日志
            // m_batchDialog->closeWorkerTab(workerId);
        });
        ...
        connect(m_batchManager, &BatchAutoGenerateManager::finished,
                this, [this]() {
            m_batchDialog->appendLog("[BatchManager] ===== 批量处理完成 =====");
            m_batchDialog->setRunning(false);
            m_batchRunning = false;
            
            // 注意：不自动关闭 Worker Tabs，让用户可以查看日志
            // 用户可以手动关闭对话框或开始新的批量处理时会清理
        });
    }
...
{
    if (m_batchManager) {
        m_batchDialog->appendLog("[BatchManager] 正在停止批量处理...");
        m_batchManager->stop();
        // Manager::stop() 会发出 finished 信号，在那里重置状态
    } else {
        // 如果 Manager 不存在，直接重置状态
        m_batchRunning = false;
        m_batchDialog->setRunning(false);
    }
}
...
bool DialogUnitManage::startAutoGenerationForSelection(TModelAutoGenerator *generator,
                                                       ...
                                                       bool takeOwnership,
                                                       bool usePortsFallback)
{
    QList<QPair<QString, QString>> effectivePorts = ports;
    // 仅当 usePortsFallback=true 且 ports 为空时，才从 UI 表格收集端口
    if (effectivePorts.isEmpty() && usePortsFallback)
...
void DialogUnitManage::startNextBatchTask()
{
    ...
    // 批量模式下不再进行UI操作（不选择/切换表格项），数据完全从数据库加载
    ...
    if (m_batchGenerator) {
        ...
        // 批量模式：每次处理一个器件，但数据从数据库加载而不是从UI
        // 设置批量模式标志，避免UI操作
        m_batchGenerator->setBatchMode(true);
        m_batchGenerator->setSelectedEquipmentId(m_currentBatchTask.equipmentId);
        m_batchGenerator->setPreloadedPorts(QList<QPair<QString, QString>>());  // 清空预加载，强制数据库加载
        m_batchGenerator->setUnitManageDialog(this);
        m_batchGenerator->startAutoGeneration();
    }
}
