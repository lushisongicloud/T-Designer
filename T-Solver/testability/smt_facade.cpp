...
{
    return z3Solve(smtCode, timeoutMs);
}
...
QString SmtFacade::joinAssertions(const QStringList &assertions) const
{
    ...
    for (const auto &assertion : assertions) {
        if (assertion.trimmed().isEmpty()) {
            continue;
        }
        if (assertion.trimmed().startsWith(QStringLiteral("(assert"))) {
            code += assertion;
        } else {
            code += QStringLiteral("(assert %1)").arg(assertion);
        }
    }
    return code;
}
