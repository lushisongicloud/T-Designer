...
#include "BO/batch/batchautogeneratemanager.h"
...
public:
   ...
   QSet<QString> equipmentID_IN_EquipmentTemplate;
    ...
    void regeneratePortVariables(bool showNotification = true);
    void clearTModelEditor();
private slots:
...
private:
    ...
    struct BatchTask {
        int equipmentId = 0;
        QPersistentModelIndex categoryIndex;
        QString code;
        QString name;
    };
    struct BatchCategoryState {
        QPersistentModelIndex categoryIndex;
        QList<BatchTask> tasks;
        int cursor = 0;
    };
    BatchAutoGenerateDialog *m_batchDialog = nullptr;
    TModelAutoGenerator *m_batchGenerator = nullptr;
    BatchAutoGenerateManager *m_batchManager = nullptr;  // 新的批量管理器
    QList<BatchTask> m_batchTasks;
    QList<BatchCategoryState> m_batchCategoryStates;
    BatchTask m_currentBatchTask;
    int m_batchTotalTasks = 0;
    int m_batchCompletedTasks = 0;
    int m_batchCycleCount = 0;
    bool m_batchRunning = false;
    bool m_batchGeneratorBusy = false;
    QString m_batchLogPath;
    QFile m_batchLogFile;
    QTextStream m_batchLogStream;
    QSet<int> m_batchCompletedSet;

    void startBatchAutoGeneration(const QString &logPath, bool resume);
    void startBatchAutoGenerationNew(const QString &logPath, bool resume);  // 新实现
    void stopBatchAutoGenerationNew();  // 停止新实现
    void prepareBatchTasks(bool resume);
    QList<QPersistentModelIndex> collectLeafCategoryIndexes() const;
    bool selectCategoryIndex(const QPersistentModelIndex &index);
    bool selectEquipmentInTable(int equipmentId);
    QList<QPair<QString, QString>> collectPortsFromTableTerm() const;
    void startNextBatchTask();
    void finishBatchAutoGeneration(const QString &message);
    void cancelBatchAutoGeneration();
    void appendBatchLog(const QString &line, bool persist = false);
    QString makeBatchLogPath() const;
    void recordBatchResult(int equipmentId, const QString &code, const QString &status, const QString &message);
    void handleBatchGeneratorFinished();
    void handleBatchComponentFinished(int equipmentId, const QString &code, bool success, const QString &message);
    void handleBatchLogLine(const QString &line);
    bool refillBatchQueue();
    bool startAutoGenerationForSelection(TModelAutoGenerator *generator,
                                         int equipmentIdOverride,
                                         const QList<QPair<QString, QString>> &ports,
                                         bool takeOwnership,
                                         bool usePortsFallback = true);
    bool ensureDeepSeekApiKey() const;
    bool loadCompletedFromLog(const QString &logPath);
    void focusFirstPendingEquipment();
