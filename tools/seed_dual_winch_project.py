#!/usr/bin/env python3
"""
Seed MyProjects/双电机拖曳收放装置/双电机拖曳收放装置.db with a curated,
...
PROJECT_DB = Path("MyProjects/双电机拖曳收放装置/双电机拖曳收放装置.db")
REF_DB = Path("ref/LdMainData.db")

TARGET_COMPONENTS = 4122
TARGET_CONNECTIONS = 6831
TARGET_DETECTION_RATE = 0.9678
TARGET_FUZZY_GROUP_COUNTS = [7911, 1475, 1122, 316, 69, 40, 20]
...
BASE_CONTAINERS = [
    dict(key="system", name="双电机拖曳收放装置", level="system", project_structure_code="WH-SYS", parent=None, description="系统根容器"),
    dict(key="control", name="主控制柜", level="subsystem", project_structure_code="WH-CTRL", parent="system", description="控制柜"),
    dict(key="distribution", name="配电段", level="subsystem", project_structure_code="WH-DIST", parent="control", description="配电与电源滤波段"),
    dict(key="hydraulic", name="动力执行段", level="subsystem", project_structure_code="WH-HYD", parent="system", description="动力执行段总成"),
    dict(key="sensor", name="传感器网络", level="subsystem", project_structure_code="WH-SENS", parent="hydraulic", description="传感器层"),
    dict(key="network", name="控制网络区", level="subsystem", project_structure_code="WH-NET", parent="system", description="控制与通讯网络"),
]
...
POSITION_CODES: Sequence[Tuple[str, str, str]] = [
    ("POS-CTRL-A", "控制柜A区", "WH-CTRL"),
    ("POS-CTRL-B", "控制柜B区", "WH-CTRL"),
    ("POS-DIST-A", "配电A区", "WH-DIST"),
    ("POS-HYD-A", "动力段A区", "WH-HYD"),
    ("POS-SENS-A", "传感器A区", "WH-SENS"),
    ("POS-NET-A", "控制网络间", "WH-NET"),
]

PAGE_DEFS = {
    "control": dict(
        page_id=7201,
        structure_code="WH-CTRL",
        name="控制柜布线",
        desc="控制柜主回路",
        gaoceng="WH-CTRL",
        position="POS-CTRL-A",
...
CATEGORIES: Sequence[CategoryConfig] = [
    CategoryConfig("relay", "继电器", "KA", 2, 1, 50, "WH-CTRL", "control", "control", ["继电器", "relay"], "control", "控制线"),
    CategoryConfig("contactor", "接触器", "KM", 2, 1, 45, "WH-CTRL", "control", "control", ["接触器", "contactor"], "power", "动力线"),
    CategoryConfig("breaker", "断路器", "QF", 2, 1, 45, "WH-DIST", "distribution", "distribution", ["断路器", "breaker"], "power", "动力线"),
    CategoryConfig("solenoid", "开关电磁铁", "YV", 3, 301, 70, "WH-HYD", "hydraulic", "hydraulic", ["电磁铁", "阀"], "hydraulic", "液压控制"),
    CategoryConfig("switch_sensor", "开关量传感器", "SQ", 3, 301, 55, "WH-SENS", "sensor", "sensor", ["开关", "行程"], "signal", "离散信号"),
    CategoryConfig("analog_sensor", "模拟量传感器", "SA", 3, 301, 55, "WH-SENS", "sensor", "sensor", ["sensor", "传感"], "signal", "模拟信号"),
    CategoryConfig("proportional_solenoid", "比例电磁铁", "BT", 3, 301, 25, "WH-HYD", "hydraulic", "hydraulic", ["比例", "伺服"], "hydraulic", "液压控制"),
    CategoryConfig("filter", "电源滤波器", "PF", 2, 1, 20, "WH-DIST", "distribution", "distribution", ["滤波"], "power", "动力线"),
    CategoryConfig("plc", "PLC 控制器", "PLC", 2, 1, 18, "WH-CTRL", "control", "control", ["PLC", "controller"], "control", "控制线"),
    CategoryConfig("meter", "电表", "EM", 2, 1, 18, "WH-DIST", "distribution", "distribution", ["meter", "计量"], "signal", "监测线"),
    CategoryConfig("amplifier", "放大板", "AM", 2, 1, 18, "WH-CTRL", "control", "control", ["放大", "amplifier"], "control", "控制线"),
    CategoryConfig("network_module", "网络模块", "NET", 2, 1, 18, "WH-NET", "network", "network", ["网络", "通信", "module"], "network", "通讯线"),
]
...
def reset_autogen_data(conn: sqlite3.Connection) -> None:
    ...
    if table_exists(conn, "equipment_containers"):
...
def ensure_structure_codes(conn: sqlite3.Connection) -> Dict[str, int]:
    ...
    for code, desc in HIGH_LEVEL_CODES:
        structure_map[code] = ensure_entry("3", code, desc, root_id)

    for code, desc, parent_code in POSITION_CODES:
        parent_id = structure_map.get(parent_code)
        if parent_id is None:
            raise RuntimeError(f"未找到高层代号 {parent_code} 的 ProjectStructure 记录。")
        structure_map[code] = ensure_entry("5", code, desc, parent_id)
...
def ensure_base_containers(conn: sqlite3.Connection, structure_map: Dict[str, int]) -> Dict[str, int]:
    ...
    for spec in BASE_CONTAINERS:
        structure_code = spec["project_structure_code"]
        structure_id = structure_map.get(structure_code)
        if structure_id is None:
            raise RuntimeError(f"未找到结构代号 {structure_code} 的 ProjectStructure 记录。")
        if spec["name"] in existing:
            container_id = existing[spec["name"]]
            conn.execute("UPDATE container SET project_structure_id=? WHERE container_id=?", (structure_id, container_id))
        else:
...
def main() -> None:
    ...
    try:
        ...
        structure_map = ensure_structure_codes(project_conn)
        for page_info in PAGE_DEFS.values():
            page_spec = page_info.copy()
            structure_code = page_spec.get("structure_code")
            if not structure_code or structure_code not in structure_map:
                raise RuntimeError(f"未找到页面 {page_spec['name']} 的结构代号 {structure_code}")
            page_spec["structure_id"] = structure_map[structure_code]
            ensure_page(project_conn, page_spec)
        base_container_ids = ensure_base_containers(project_conn, structure_map)
        ...
        for config in CATEGORIES:
            structure_id = structure_map.get(config.project_structure_code)
            if structure_id is None:
                raise RuntimeError(f"未找到类别 {config.key} 对应的结构代号 {config.project_structure_code}")
            ...
            for idx in range(actual_count):
                ...
                if table_exists(project_conn, "equipment_containers"):
                    project_conn.execute(
